...
実装済み: ログイン・書棚・詳細(G0)/ AVPlayer再生+ロック画面統合(G1)/
スライド同期・章一覧・速度(G2)/ 進捗の端末間同期(G3)/
オフライン一括保存とオフライン再生(G4)。残り: TestFlight配布(G5)。

ライセンス: リポジトリ全体と同じ AGPLv3。App Store/TestFlight 配布のための
第7条追加許可を `COPYING.iOS` に置く。
