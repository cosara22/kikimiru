...
- `slides[].image`(v2.1・任意): 画像スライド。ブックフォルダ内の画像ファイルへの
  相対パス(例 `"slides/s01.png"`。**最大2セグメント**・各セグメントは `.` 始まり/`..`/`\`
  を含めない・拡張子は `.png` `.jpg` `.jpeg` `.webp`)。指定するとプレイヤーはテキスト面の
  代わりに画像を全面表示(レターボックス)する。**画像スライドにも必ずセマンティックレイヤー
  (content.json 側の `alt` / `bullets`)を併記すること** — 検証で警告される(下記)
...
- `alt`(v2.1・任意): 画像スライドの**セマンティックレイヤー**。図・グラフ・レイアウトが
  何を示しているかのテキスト説明。プレイヤーは `<img>` の alt(読み上げ)と画像ロード失敗時の
  フォールバック表示に使う。検索の対象にもなる

#### 画像スライドのセマンティックレイヤー(規約)

画像スライド(`slides[].image`)を使う場合も、**内容の意味は必ずテキストとして併記する**:
`title`(見出し)+`alt`(図・グラフの説明)または `bullets`(要点)。
画像だけのスライド(ピクセルのみで意味がテキスト化されていない状態)は
`tools/validate_deck.py` が警告(WARN)を出す。目的は3つ —
(1) スクリーンリーダー・読み上げ環境での可用性
(2) 検索・絞り込みの対象化
(3) 共有・エクスポート時のテキスト代替(画像は本文と同様ローカル側の資産という整理)
